...
mysql_select_db($database_MAMP, $MAMP);
$query_qUsers = "SELECT * FROM Users WHERE `role` = 'Client' ORDER BY firstName ASC";
$qUsers = mysql_query($query_qUsers, $MAMP) or die(mysql_error());
$row_qUsers = mysql_fetch_assoc($qUsers);
$totalRows_qUsers = mysql_num_rows($qUsers);
    ...
    <div id="contentRight">
      <form id="updateForm" name="updateForm" method="post" action="">
        ...
        <table width="761" height="90" border="0">
          ...
          <tr class="table">
            <td width="139" height="58" bgcolor="#CCCCCC">Username</td>
            <td width="145" bgcolor="#CCCCCC">First Name</td>
            <td width="141" bgcolor="#CCCCCC">Last Name</td>
            <td width="151" bgcolor="#CCCCCC">NHI Number</td>
            <td width="155" bgcolor="#CCCCCC">Service ID</td>
          </tr>
          <?php do { ?>
            <tr>
              <td height="65"><?php echo $row_qClient['Username']; ?></td>
              <td><?php echo $row_qClient['FirstName']; ?></td>
              <td><?php echo $row_qClient['LastName']; ?></td>
              <td><?php echo $row_qClient['NHINumber']; ?></td>
              <td><?php echo $row_qClient['ServiceID']; ?></td>
            </tr>
            <?php } while ($row_qClient = mysql_fetch_assoc($qClient)); ?>
          <tr>
            <td colspan="5" align="right" valign="bottom"><a href="<?php printf("%s?pageNum_qClient=%d%s", $currentPage, max(0, $pageNum_qClient - 1), $queryString_qClient); ?>">Previous</a> | <a href="<?php printf("%s?pageNum_qClient=%d%s", $currentPage, min($totalPages_qClient, $pageNum_qClient + 1), $queryString_qClient); ?>" class="table">Next </a></td>
          </tr>
        </table>
        <p>&nbsp;</p>
      </form>
